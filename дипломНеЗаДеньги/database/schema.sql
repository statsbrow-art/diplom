...
-- 13. Таблица адресов пользователей
...
-- 15. Таблица подписок на рассылку
...
-- Триггер для логирования активности при создании заказа
CREATE OR REPLACE FUNCTION log_order_activity()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO activity_logs (user_id, action, entity_type, entity_id, details)
    VALUES (NEW.user_id, 'order_created', 'order', NEW.id, 
            jsonb_build_object('order_number', NEW.order_number, 'total', NEW.total_price));
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Представление популярных категорий
